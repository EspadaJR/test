...
import 'package:flutter/services.dart' show rootBundle, SystemNavigator;  // Am adăugat SystemNavigator aici
  ...
  void _confirmQuit(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(getString('quitConfirmTitle') ?? 'Confirmare ieșire'),
        content: Text(getString('quitConfirmMessage') ?? 'Sigur vrei să ieși din aplicație?'),
        actions: [
          TextButton(
            child: Text(getString('cancel') ?? 'Anulează'),
            onPressed: () => Navigator.of(context).pop(),
          ),
          TextButton(
            child: Text(getString('quit') ?? 'Ieșire'),
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ElevatedButton.icon(
              style: buttonStyle.copyWith(backgroundColor: MaterialStateProperty.all(Colors.green.shade700)),
              icon: const Icon(Icons.camera_alt),
              label: Text(getString('scan')),
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => ScanPage(getString: getString)));
              },
            ),
            const SizedBox(height: 15),
            ElevatedButton.icon(
              style: buttonStyle.copyWith(backgroundColor: MaterialStateProperty.all(Colors.blue.shade700)),
              icon: const Icon(Icons.list),
              label: Text(getString('mushroomList')),
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(
                  builder: (context) => MushroomListPage(
                    getString: getString,
            ...
            const SizedBox(height: 15),
            ElevatedButton.icon(
              style: buttonStyle.copyWith(backgroundColor: MaterialStateProperty.all(Colors.orange.shade700)),
            ...
            const SizedBox(height: 15),
            ElevatedButton.icon(
              style: buttonStyle.copyWith(backgroundColor: MaterialStateProperty.all(Colors.purple.shade700)),
              icon: const Icon(Icons.quiz),
              label: Text(getString('quiz')),
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(
                  builder: (context) => QuizPage(getString: getString),
                ));
              },
            ),
            const SizedBox(height: 15),
            ElevatedButton.icon(
              style: buttonStyle.copyWith(backgroundColor: MaterialStateProperty.all(Colors.red.shade700)),
              icon: const Icon(Icons.exit_to_app),
              label: Text(getString('quit')),
...
class ScanPage extends StatelessWidget {
  final String Function(String) getString;
  const ScanPage({super.key, required this.getString});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(getString('scan'))),
      body: const Center(child: Text('Scanarea va fi implementată aici...')),
    );
  }
}

// Asigură-te că ai definit clasa MushroomListPage, SettingsPage, QuizPage în alte fișiere sau în același fișier (nu le pune de două ori!)
  ...
  'ro': {
    ...
    'changeTheme': 'Schimbă tema',
     'scan': 'Scanează',
      'quiz': 'Quiz',
      'quit': 'Ieșire',
      'quitConfirm': 'Ești sigur că vrei să ieși din aplicație?',
      'yes': 'Da',
      'cancel': 'Nu'
  },
  'en': {
    ...
    'changeTheme': 'Change Theme',
      ...
      'quiz': 'Quiz',
      'quit': 'Quit',
      'quitConfirm': 'Are you sure you want to exit the app?',
      'yes': 'Yes',
      'cancel': 'No',
  },
  'de': {
    ...
    'changeTheme': 'Thema ändern',
      ...
      'quiz': 'Quiz',
      'quit': 'Beenden',
      'quitConfirm': 'Möchten Sie die App wirklich beenden?',
      'yes': 'Ja',
      'cancel': 'Nein',
  },
};
...
void main() async {
  ...
  runApp(const MushroomScannerApp());
}
...
class _MushroomScannerAppState extends State<MushroomScannerApp> {
  ...
  void initState() {
    ...
    String langCode = settingsBox.get('locale', defaultValue: 'ro');
